...
public class AuthService(DataContext context, ILogger<AuthService> logger, IEmailService emailService, IHashService hashService, Microsoft.Extensions.Configuration.IConfiguration configuration) : IAuthService
{
    ...
    public async Task<Response<string>> Register(RegisterDto registerDto)
    {
        try
        {
            ...
            return new Response<string>($"Successfully created new user by Id = {user.Id}");
        }
        catch (Exception e)
        {
            logger.LogError("Exception {Exception} in time:{DateTime} ", e.Message, DateTimeOffset.UtcNow);
